...
| `drinkConsumeMinutes`    | integer               | Default `20`. How long, in minutes, the user typically takes to consume one drink — the user-facing half of the BAC absorption-window model (see [party-session.md → Drink consumption time](./party-session.md#drink-consumption-time)). Range `0`–`60`, editable in 5-minute steps. Mirrored onto `PartySession.drinkConsumeMinutes` at session start and live-updated onto the active session while it runs; frozen once the session ends. Editable any time from Settings → Party Mode, independent of whether a session is active. |
...
| `drinkConsumeMinutes` | integer         | Copied from `UserPreferences.drinkConsumeMinutes` when the session starts. While the session is active (`endedAt IS NULL`), editing the global preference — or the session's own live control on the Party tab — updates this field directly, and every drink's BAC contribution recomputes against the new value immediately (this is a live read at computation time, not a per-`DrinkEntry` snapshot — there is nothing to sweep retroactively). Once `endedAt` is set, this field no longer changes, even if the global preference changes afterward — it holds whatever value was in effect at the moment the session ended. See [party-session.md → Drink consumption time](./party-session.md#drink-consumption-time). |
| `createdAt`   | timestamp             | Record creation time.                                                                |
