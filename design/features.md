...
- **Drink consumption time.** A global "time to consume a drink" setting (default 20 min) feeds an absorption window that spreads each drink's BAC contribution over time instead of applying it instantly — combined with a fixed 30-minute post-swallow absorption delay. The setting is mirrored onto the active session and can be changed live from either Settings or the Party tab while the session runs; it freezes at the value in effect the moment the session ends. See [party-session.md → Drink consumption time](./party-session.md#drink-consumption-time).
