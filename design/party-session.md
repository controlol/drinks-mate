...
- A small **meal indicator** showing the most recent meal logged in this session (size + relative time, plus a trailing "· Add" to signal intent, e.g. "Medium meal · 2 h ago · Add"). Tapping the indicator opens the meal-size prompt to log a new meal. If no meal has been logged, the control reads "Add meal" (no suffix needed — the label already says "Add"). Editing an already-logged meal is done from [S9 Party Session Log](./user-experience.md#s9--party-session-log), the single authoritative place for meal edits, matching the drink-editing model.
