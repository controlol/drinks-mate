...
- A **day list** below the charts. Tapping a day on any chart, or selecting a row in the list, drills into the day detail (drink list with edit/delete, plus any Party Session summary on that day). Editable fields are volume, name, ABV (alcoholic drinks only), price, and time — S3 is the only screen that exposes name; S6 and S9 do not (see their own entries below). An alcoholic drink attached to a Party Session (`partySessionId` set) is read-only here — edit or delete it from [S9 Party Session Log](#s9--party-session-log) instead, the single authoritative place for session-attached drinks. Delete is a soft-delete with confirmation.
