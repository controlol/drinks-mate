...
- **Swipe to change day, from anywhere in the day drill-down.** Once inside the day detail (however it was reached — chart tap or day-list row tap), a horizontal swipe anywhere on the screen — over the totals header, a session card, or the drink list, at any scroll position — steps to the adjacent day: swipe back for the previous day, forward for the next, one day per gesture. This is in addition to, not a replacement for, reaching a specific day by tapping a chart bar or list row. The AppBar's date title updates immediately to the day now showing.
  - **Independent of the range selector.** The drill-down keeps its own day cursor; swiping across a week or month boundary just continues day-by-day and does not page the Weekly/Monthly range selector underneath. Returning to History leaves that selector exactly where it was when the user left.
  - **Bounds.** Forward is bounded at **today's day-window** — never into the future, consistent with the entry edit sheet's own date-picker bound above. Backward is bounded at the **earliest day-window with anything to show**: the earliest of any `DrinkEntry.consumedAt`, `PartySession.startedAt`, or `UserPreferences.installedAt` (see [data-model.md → UserPreferences](./data-model.md#userpreferences)) across the local database — so paging can never run off into an indefinite run of days that could not possibly have data. Swiping past either bound is a no-op; the gesture is absorbed with a subtle resistance rather than wrapping around or erroring.
  - **Dataless days still render.** A day within bounds but with no drinks and no session shows the same empty state used today rather than being skipped — swiping always moves exactly one calendar day per gesture, never more.
  - **Multi-day sessions unaffected.** Each day's Party Session summary card is computed exactly as described below, day-clipped to whichever day is currently showing — the "Day N of M" pill, expand behaviour, and "View full session" link keep working unchanged as the user swipes across the days a session touches.
  - **Motion.** A calm horizontal slide between days, ease-in-out, with no bounce or overshoot at the bounds — overriding platform-default edge-bounce scroll physics, per [engineering/phase-1-constraints.md → C5](../engineering/phase-1-constraints.md#c5--design-system-visual--behavioural-parity-by-construction) and [designer-brief.md → Motion & feedback](./designer-brief.md#motion--feedback). A reduce-motion fallback replaces the slide with an instant cut, never a scale/parallax substitute.
  - Backed by a `core` day-paging function mirroring `pagedIsoWeekWindow`/`pagedMonthWindow` (e.g. `pagedDayWindow({required now, required int offset, ...})`), so day-stepping reuses the same `[boundary, next boundary)` semantics as the rest of History rather than new ad-hoc date math — see [flutter/packages/core/lib/src/day_boundary.dart](../flutter/packages/core/lib/src/day_boundary.dart).
...
5. **Party Mode**
   ...
   - Time to consume a drink (minutes, default 20, range 0–60 in 5-minute steps) — see [party-session.md → Drink consumption time](./party-session.md#drink-consumption-time). While a session is active, this same value is also editable inline from the Party tab; editing it in either place updates the active session immediately.
