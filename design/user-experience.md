...
- **The day's Party Session summary is tappable, and expands in place.** Collapsed, it shows the fields it always has (duration, alcoholic drinks count, meals count, peak estimated BAC — see [`SessionSummaryCard`](../flutter/lib/src/widgets/session_summary_card.dart)), all **day-clipped** as today (a session spanning multiple days gets one card per day it touches, each scoped to that day's window — see `SessionDaySummary`/`buildSessionDaySummary`).
  - **Multi-day indicator.** When a session spans more than one calendar day, every card it touches carries a **"Day N of M"** label (e.g. "Day 2 of 3") in the **header row itself**, between the session name and the expand chevron — the same trailing slot [S9](#s9--party-session-log)'s header uses for its edit-name affordance. The two never collide: S3 never wires that slot to name-editing (name is display-only here — see the entry-row bullet above), so it is free for this label; S9 never shows the badge (it's never day-clipped — see below). Visible in **both** collapsed and expanded states, since it's part of the always-visible header row — so the partial, day-sliced nature of the numbers below it is obvious before the user even taps to expand. `N` is this card's position (1-indexed, chronological) among the calendar days the session touches; `M` is the total count of days it touches. A single-day session shows no badge at all, and the session name keeps the row's full remaining width exactly as today.
