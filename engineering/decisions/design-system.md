...
**Rationale.** C4 has many branch points — the Watson-vs-Widmark selection, the meal modifier (exponential decay, **min** across meals), zero-order elimination (β=0.15), pooled multi-drink elimination, g/L→mmol/L (×21.7), the unspecified-gender conservative path, the BMI-range warning, the orphan `t_zero` absorption rule, and the lazy 12-hour auto-end. Each is a place a number can be subtly wrong. Implementing them once with the Rulebook's pinned rounding, and pinning each with a fixture, turns a wrong number into a failing test.
...
| **BAC: absorption window** | `T_hours = (drinkConsumeMinutes + ABSORPTION_DELAY_MINUTES) / 60`, `ABSORPTION_DELAY_MINUTES = 30` (fixed, not user-configurable). `drinkConsumeMinutes` comes from the owning `PartySession` (mirrors `UserPreferences.drinkConsumeMinutes` while active, frozen at `endedAt`); default **20**, range **0–60**, 5-minute steps. `T_hours` is never 0 — the 30-minute constant always applies. Per-drink absorption rate `r = BAC_initial / T_hours` (g/L per hour), constant over `[consumedAt, consumedAt + T_hours)`. | party-session Step 4; §Drink consumption time |
| **BAC: elimination rate** | β = **0.15 g/L per hour** (default), applied whenever alcohol is in the blood, including while a drink is still absorbing. Typical range 0.10–0.20 g/L/h. | party-session Step 5 |
| **BAC: pooled current value** | `net_rate(t) = (Σ r_i for every drink i currently absorbing at t) − β`, floored at 0. Compute by walking a sorted timeline of rate-change events (`+r_i` at each drink's `consumedAt`, `−r_i` at `consumedAt + T_hours`) and integrating piecewise-linearly up to the query time, flooring the running pool at 0 after each segment. Degrades to the pre-absorption-window running-pool loop ("decay at β, then add `BAC_initial`") as every `T_hours_i → 0`. One shared β at every instant — never `N × β` for `N` concurrently-active drinks. | party-session Step 6 |
| **BAC → mmol/L** | `mmol_per_L = g_per_L × 21.7`. g/L is canonical; mmol/L is display-only. | party-session Step 7; C4 |
| **Orphan absorption** | per orphan: `t_zero = consumedAt + BAC_initial / β` when that drink's `r > β` (Step 6's emergent property: this total duration is unaffected by the absorption window). When `r <= β`, `t_zero = consumedAt` (never accumulates residual BAC). Absorb iff `t_zero > new session startedAt`, else stays decayed orphan. | party-session §Absorbing orphan drinks |
...
| **Approaching-cap trigger** | evaluated once per drink log: **project the pool forward** through every currently-absorbing drink's remaining absorption window (assuming no further drinks), find the projected peak, and fire if `projected_peak_gPerL >= 0.8 * capGPerL` (inclusive boundary). Reuses the BAC line chart's dashed-segment projection rather than a separate computation. Changed from the pre-absorption-window "instant value right now" reading, which stopped being meaningful once Step 4 spread a drink's effect over a window. | party-session §Notifications during a session |
...
| **Metric display precision — single-drink volume** | Round to the nearest **integer ml**. Applies to per-entry displays (S6 list, log/edit sheets). | features F6; data-model §Units |
| **Metric display precision — daily-progress headline volume** | Always convert to **litres**, regardless of magnitude — no ml fallback below 1 000 ml — to **1 decimal place, trailing `.0` omitted for whole litres** (e.g. "0.2 L", "1.4 L", "2 L"). Applies to today's intake/goal, history totals/averages, and the daily-goal setting caption. | data-model §Units §Display precision |
| **Metric display precision — mass & height** | Metric mass: **1 decimal place** (kg). Metric height: **1 decimal place** (cm). | features F6; data-model §Units |
...
| **Preset sort mode** | `manual`: `DrinkPreset.sortOrder` ascending. `recentlyUsed` (default): rank by `MAX(consumedAt)` over non-deleted `DrinkEntry` rows with matching `presetId`, descending; presets with no match sort last. `mostUsed`: rank by `COUNT(*)` over non-deleted `DrinkEntry` rows with matching `presetId` **and** `consumedAt` in the trailing 30-day window `[now − 30d, now)`, descending; presets with zero matches sort last. **Both modes key off `consumedAt`, never `createdAt`** — a retroactively-backdated entry ranks as if logged at its `consumedAt` time, consistent with every other stat in the app being bucketed by `consumedAt`. **Ties in `recentlyUsed`/`mostUsed` (including the no-match/zero-count group) break by `sortOrder` ascending.** | features F14 §Sort modes; data-model §DrinkEntry, §UserPreferences |
...
| **PartySession name** | Optional, freeform (deliberately **not** the username/tokenName structural whitelist — natural-language names need spaces and apostrophes, e.g. "Sarah's birthday"). Trim leading/trailing whitespace; empty-after-trim stores as `null`. Max 40 characters after trimming. Only control chars (`Cc`) are stripped; everything else is allowed. | data-model §PartySession |
| **BAC chart empty-state window** | Before the first alcoholic drink in a session, the chart X-axis spans `startedAt` to `startedAt + 3h` (fixed 3-hour default), flat `0.00 g/L` line, no dashed projection. Replaced by the real projected-end-time axis the moment the first drink logs. | party-session §BAC line chart → Empty state |
| **Zero-drink session discard threshold** | A session is discarded (soft-deleted, no confirmation) if it has **exactly 0** alcoholic drinks (in-session + absorbed orphans) at the moment it would end, by either `manual` or `auto_timeout`. `≥ 1` drink always persists the session normally. | party-session §Zero-drink sessions are never saved |
| **`drinkConsumeMinutes` bounds** | Integer minutes, range **0–60**, editable in **5-minute steps**. Default **20**. Global (`UserPreferences`), mirrored onto `PartySession` while active, frozen at `endedAt`. | party-session §Drink consumption time |
