...
> **Updated for the Flutter direction.** This document was written when Drinks Mate was two native codebases, and its job then was "how do we share *specifications* so two independent implementations stay in lockstep." With the switch to a **single Flutter codebase** ([flutter-stack.md](./flutter-stack.md)), most of that need evaporates: there is one widget tree and one `core` package, so visual and behavioural parity hold **by construction**, not by governance. What survives — and is the lasting value of this doc — is the **token source of truth** (one set of design tokens now feeding one Flutter `ThemeData` instead of two native artifacts) and the platform-neutral **[Parity Rulebook](#appendix--parity-rulebook)**, which remains the **canonical spec of every rounding, unit, boundary, and formula constant** the single implementation must meet. Read the D1–D5 sections below as describing *one* implementation: where they say "both native suites / both platforms," substitute "the one Flutter app and its `core` unit tests." The Style-Dictionary-to-native-artifacts mechanics (D1) and the KMP-vs-dual-implementation trade (D4) are now moot in their original form and annotated as such.

The original framing: a three-legged stool of (1) a **platform-neutral design-token source of truth** (colour/spacing/type/motion), (2) **DM Sans** with tabular figures, and (3) a spec + fixtures for the high-risk C4 computation, held together by a governance process and the **Parity Rulebook**. Under Flutter, (1) compiles to a single Flutter theme, (2) is bundled once, and (3) collapses to one Dart implementation with the fixtures kept as regression tests. This document is written against [phase-1-constraints.md](../phase-1-constraints.md) (especially C4, C5, C6) and the [engineering README](../README.md).

## Decisions at a glance

| # | Decision | Choice | Confidence |
| - | -------- | ------ | ---------- |
| D1 | Design tokens as single source of truth | **Design tokens (DTCG-format JSON)** → one Flutter `ThemeData` (one source of truth; under Flutter no native-artifact split is needed) | High |
| D2 | Typography | **DM Sans** (OFL-1.1 variable font) bundled once; `tnum` tabular figures forced for headline numerics; honours OS text scaling | High |
...
| D4 | Shared computation | **One pure-Dart `core` implementation**; design worked-examples kept as regression unit tests (cross-platform golden-vector gate and KMP trade now moot — single codebase) | High |
