...
| **BAC: elimination** | `BAC(t) = max(0, BAC_initial − β × (t − t_drink))`, β = **0.15 g/L per hour**, for a single drink. **Multiple drinks pool into one running total** — not summed independently: process drinks in consumption order, decaying the pool at β for elapsed time since the previous drink (floored at 0) before adding the next drink's `BAC_initial`; current estimate is that pool decayed at β from the last drink to now (floored at 0). Independent per-drink summation over-eliminates by β per additional concurrently-active drink. | party-session Steps 4–5 |
...
| **Approaching-cap trigger** | fires when a logged drink pushes estimated BAC to **≥ 80%** of `bacCapGramsPerL` (boundary is inclusive: `bacGPerL >= 0.8 * capGPerL`) — reaching the threshold counts as approaching it, consistent with the app's conservative-estimate posture elsewhere (e.g. unspecified-gender). | party-session §BAC goal |
