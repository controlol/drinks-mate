...
| **Metric display precision — single drink** | Metric volume in single-drink / entry-field display: round to the nearest **integer ml** (e.g. `"240 ml"`). | features F6; data-model §Units |
| **Metric display precision — daily-progress headline** | Metric volume in daily-progress headline display (today card, history totals): `< 1 000 ml` → nearest **integer ml**; `≥ 1 000 ml` → divide by 1 000 and format to **1 decimal place** (round-half-away-from-zero, `toStringAsFixed(1)`), omitting the trailing `.0` for whole-litre values (e.g. 1 400 ml → `"1.4 L"`, 2 000 ml → `"2 L"`). | D2; data-model §Units |
