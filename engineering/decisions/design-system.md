...
| **Metric display precision — single drink / entry field** | Volume: nearest **integer ml** (e.g. `"240 ml"`). | features F6; data-model §Units |
| **Metric display precision — daily-progress headline** | Volume (today card, history totals): always **1 dp in litres** (`ml / 1000`, `toStringAsFixed(1)`), trailing `.0` omitted (e.g. `"0.5 L"`, `"1.4 L"`, `"2 L"`). No ml fallback — litres at all values. Rounding via Dart `toStringAsFixed` (round-half-away-from-zero). | D2 typography; data-model §Units |
| **Metric display precision — mass & height** | Mass: **1 decimal place** (kg). Height: **1 decimal place** (cm). | features F6; data-model §Units |
