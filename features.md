...
This document lists the functional scope of Drinks Mate, organised by phase. The phase boundary is described in [technical-architecture.md](technical-architecture.md): phase 1 must ship as a complete product on its own.
...
- **Hydration per day** — bar chart of total intake in litres for each day in the range, with the user's daily goal drawn as a horizontal reference line. Bars below the line are visually distinct from bars at or above it (using a non-colour signal too, per the accessibility rule in [user-experience.md](user-experience.md)).
...
Maximum-BAC bars are clearly labelled as estimates everywhere they appear — same disclaimer rule as the live BAC display in [party-session.md](party-session.md).
...
- All data is stored in a **local database** on the device. See [data-model.md](data-model.md).
...
- **Name** — required, free text. Same character rules as username (3–30 chars, see [data-model.md](data-model.md)). `[OPEN]` — confirm length range for preset names; could be more permissive than usernames.
...
- **Regular price** — optional. The user's normal-day "menu price" for this drink. Stored per-preset together with the **currency** in which the price was entered. The user's preferred currency (set in settings) is used as the default for new presets but is per-preset and per-entry on the record. Phase 1 supports `EUR`, `USD`, and `GBP`. The app does not convert between currencies and does not track exchange rates — see [data-model.md](data-model.md) "Currency". Price + currency are copied to each `DrinkEntry` at log time so historical totals stay accurate even if the preset or the user's preferred currency changes later. Per-session override prices (festival / party prices) live separately on the session — see F12 and [party-session.md](party-session.md).
...
Resolved values (name, volume, ABV, price, icon, colour) are **snapshotted** onto each `DrinkEntry` at log time. Editing or deleting a preset later does **not** modify already-logged entries. See [data-model.md](data-model.md).
...
- The local database remains the source of truth on each device. The server reconciles between devices using the rules in [technical-architecture.md](technical-architecture.md) and [data-model.md](data-model.md).
